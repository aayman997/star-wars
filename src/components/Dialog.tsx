...
const ModalContext = createContext<DialogContextType | object>({});
...
const Open = ({ children, opens: opensWindowName }: DialogOpenPropsType) => {
	const { open } = useContext<DialogContextType>(ModalContext);
...
function Window({ children, name }: DialogWindowPropsType) {
	const { openName, close } = useContext<DialogContextType>(ModalContext);
	...
	return createPortal(
		<div className="fixed inset-0 z-10 bg-black/50 backdrop-blur-sm transition-all duration-500">
			<div
				className="fixed left-1/2 top-1/2 w-1/3 -translate-x-1/2 -translate-y-1/2 rounded-xl bg-brand-50 px-5 py-5 shadow-2xl transition-all
				duration-500"
				ref={ref}
			>
